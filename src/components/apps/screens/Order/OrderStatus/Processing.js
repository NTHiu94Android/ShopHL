...
const Item = ({ item, onpress }) => (
  <View style={styles.containerItem}>
    ...
    <View style={[styles.rowItem, { marginTop: 16 }]}>
      ...
      <Text style={{ fontSize: 16, fontWeight: '600', color: '#FF4500' }}>{item.status}</Text>
    </View>
  </View>
);
...
const Processing = (props) => {
  ...
  const getOrderByIdUserAndStatus = async () => {
    const res = await onGetOrderByIdUserAndStatus(user._id, 'processing'); 
    console.log(res); 
    setListProcessing(res);
  };
  return (
